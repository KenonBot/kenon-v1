...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!emoji) {
      return interaction.reply('No emoji provided. Please provide a valid Discord emoji.');
    }

    const match = emoji.match(/^<a?:\w+:(\d+)>$/);
    if (!match) {
      return interaction.reply('Invalid emoji provided. Please provide a valid Discord emoji.');
    }
    ...
    if (!supportedExtensions.includes(extension)) {
      return interaction.reply('Invalid emoji format. Supported formats are PNG, GIF, JPG, and JPEG.');
    }
