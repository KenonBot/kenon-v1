...
module.exports = {
    data: new SlashCommandBuilder()
      ...
      .addStringOption(option => option.setName('job').setDescription('The job title you want to apply for.')
        .addChoices(
    ...
    async execute(interaction) {
      ...
      try {
        ...
        if (requiredAmount) {
          if (user.successfulWorkShifts < requiredAmount) {
            const notEligible = new EmbedBuilder()
              .setColor('#2f3136')
              .setTitle('<:klist:1134998610044538890> 〢 Error')
              .setDescription(`> You need at least \`${requiredAmount}\` successful work shifts to apply for the \`${jobTitle}\` job.`);
        ...
        const existingApplication = await JobApplication.findOne({ userId, jobTitle });
        if (existingApplication) {
          const alreadyApplied = new EmbedBuilder()
            .setColor('#2f3136')
            .setTitle('<:klist:1134998610044538890> 〢 Error')
            .setDescription(`> You have already applied for the ${jobTitle} job.`);
        ...
        const applicationResult = new EmbedBuilder()
          ...
          .setDescription(`> You have applied for the \`${jobTitle}\` job. You are now able to use \`/work\`.`);
  
        interaction.reply({ embeds: [applicationResult] });
      } catch (error) {
        console.error('Error while applying for a job:', error);
        interaction.reply('> An error occurred while applying for a job. This has been logged.');
      }
    },
  };
